...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("ArpaCoin"))
        return false;
...
{
    // Convert ArpaCoin:// to ArpaCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("ArpaCoin://"))
    {
        uri.replace(0, 12, "ArpaCoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "ArpaCoin.lnk";
}
...
{
    return GetAutostartDir() / "ArpaCoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("ArpaCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  ArpaCoin-qt [" + tr("command-line options") + "]                     " + "\n";

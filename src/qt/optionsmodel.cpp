...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeArpaCoinAmount"))
        settings.setValue("nAnonymizeArpaCoinAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeArpaCoinAmount = settings.value("nAnonymizeArpaCoinAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeArpaCoinAmount"))
        SoftSetArg("-anonymizeArpaCoinamount", settings.value("nAnonymizeArpaCoinAmount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeArpaCoinAmount:
            nAnonymizeArpaCoinAmount = value.toInt();
            settings.setValue("nAnonymizeArpaCoinAmount", nAnonymizeArpaCoinAmount);
            emit anonymizeArpaCoinAmountChanged(nAnonymizeArpaCoinAmount);
            break;
        default:
            break;
        }
